...
public class AccountDBControllerTest {
	...
	/**
	 * Extra method for testing purpose
	 */
		@Test
		public void testDeleteUser() {
			controller.deleteUser("userun");
			ArrayList<Account> list = controller.getAccountList();
			for(int i=0; i <list.size(); i++){
			System.out.println(list.get(i).getUsername());
			}
			//assertTrue("the number of user in the list is 7", list.size()==8);
			assertTrue("the user in the list is" + list.get(0).getFirstName(),list.get(0).getUsername().equals("adminu"));
			assertTrue("the user in the list is" + list.get(1).getFirstName(),list.get(1).getUsername().equals("anhtran"));
			assertTrue("the user in the list is" + list.get(2).getFirstName(),list.get(2).getUsername().equals("caitlinharvey"));
			assertTrue("the user in the list is" + list.get(3).getFirstName(),list.get(3).getUsername().equals("juser"));
			assertTrue("the user in the list is" + list.get(4).getFirstName(),list.get(4).getUsername().equals("luser"));
			assertTrue("the user in the list is" + list.get(5).getFirstName(),list.get(5).getUsername().equals("nadmin"));
			assertTrue("the user in the list is" + list.get(6).getFirstName(),list.get(6).getUsername().equals("testu"));
			assertTrue("the user in the list is" + list.get(7).getFirstName(),list.get(7).getUsername().equals("useru"));
			Account account = new Account("f", "l", "userun", "p", "u", "Y", true);
			controller.addAccount(account);
			
		}

		/**
		 * this is also extra method for testing purpose
		 */
		@Test
		public void testReactivateAccount() {
			controller.reactivateAccount("useru");
			Account user = controller.getAccount("useru");
			assertTrue("the user status is N", user.getStatus().equals("Y"));
		...
		public void testAddAccount() {
			...
			assertTrue("the user password is password", anh.getPassword().equals("password"));
